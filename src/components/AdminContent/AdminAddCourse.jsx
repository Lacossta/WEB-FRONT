...
const AdminAddCourse = () => {
    ...
    // Достаем функцию управления попапом из контекста
        ...
        onSubmit: async (values, { resetForm }) => {
            if (file) {
                try {
                    ...
                    // ИСПОЛЬЗУЕМ ВАШ КОНТЕКСТ ДЛЯ УВЕДОМЛЕНИЯ
                    setInfoPopup({
                        isOpen: true,
                    ...
                    console.error(e);
                    // Показываем ошибку через попап
                    setInfoPopup({
                        isOpen: true,
                        message: "Ошибка при добавлении курса. Проверьте консоль."
                    });
                }
            } else {
            ...
            <div className={"admin-add-course"}>
                ...
                <div className={"admin-add-course__forms"}>
                    ...
                    <div className="file-upload-wrapper">
                        <label className={`admin-add-course__btn ${file ? 'active' : ''}`} htmlFor={id}>
                            {file ? 'Изменить изображение' : 'Добавить изображения'}
                        </label>
                        {file && (
                            <span className="file-success-msg">
                                ✓ Файл загружен: {file.name}
                            </span>
                        )}
                    </div>

                    <input className={"hidden"} onChange={loadImages} id={id} hidden={true} accept="image/*" type={"file"} />
